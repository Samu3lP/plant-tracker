...
function computeHealth(plant, atDate, visualScore) {
  ...
  // Misting: (times misted in last 7 days / mistsPerWeek) * 10, capped at 10
  let mistScore = 10;
  if (plant.mistsPerWeek) {
    const weekStart = new Date(new Date(atDate).getTime() - 7 * 86400000).toISOString().slice(0, 10);
    const mistCount = data.logs.filter(l =>
      l.plantId === plant.id && l.type === 'fertilise' && l.date <= atDate && l.date >= weekStart
    ).length;
    mistScore = Math.min(10, (mistCount / plant.mistsPerWeek) * 10);
  }
...
function getOverdueTasks() {
  ...
  for (const p of data.plants) {
    ...
    if (p.mistsPerWeek) {
      ...
      for (let a = 1; a <= n; a++) {
        const targetDay = Math.max(1, Math.round(a * w / (n + 1)));
        if (daysSinceCycle >= targetDay) { slotsPassed++; if (oldestSlotDay === null) oldestSlotDay = targetDay; }
      }
      if (slotsPassed > mistsInCycle) {
        const hasEverMisted = data.logs.some(l => l.plantId === p.id && l.type === 'fertilise');
        tasks.push({ plant: p, type: 'fertilise', since: daysSinceCycle, interval: oldestSlotDay || 0, neverDone: !hasEverMisted });
      }
    }
  }
...
function getNextMistLabel(plant) {
  ...
  const nextIdx = mistsInCycle;
  const daysLeft = nextIdx < slots.length
    ? slots[nextIdx] - daysSinceCycle
    : Math.max(0, w - daysSinceCycle) + slots[0];
